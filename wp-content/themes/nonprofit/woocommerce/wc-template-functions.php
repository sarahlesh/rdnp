...
function wc_products_rss_feed() {
	...
	if ( is_post_type_archive( 'product' ) || is_singular( 'product' ) ) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_attr__( 'New products', 'nonprofit' ) . '" href="' . esc_url( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_attr( sprintf( __( 'New products added to %s', 'nonprofit' ), $term->name ) ) . '" href="' . esc_url( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(__( 'New products tagged %s', 'nonprofit' ), urlencode($term->name)) . '" href="' . esc_url( $feed ) . '" />';
...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		if ( empty( $notice ) ) {
			$notice = __( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'nonprofit' );
		}
...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			$page_title = sprintf( __( 'Search Results: &ldquo;%s&rdquo;', 'nonprofit' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( __( '&nbsp;&ndash; Page %s', 'nonprofit' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
			'menu_order' => __( 'Default sorting', 'nonprofit' ),
			'popularity' => __( 'Sort by popularity', 'nonprofit' ),
			'rating'     => __( 'Sort by average rating', 'nonprofit' ),
			'date'       => __( 'Sort by newness', 'nonprofit' ),
			'price'      => __( 'Sort by price: low to high', 'nonprofit' ),
			'price-desc' => __( 'Sort by price: high to low', 'nonprofit' )
		) );
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( $post->post_content ) {
			$tabs['description'] = array(
				'title'    => __( 'Description', 'nonprofit' ),
				'priority' => 10,
		...
		if ( $product && ( $product->has_attributes() || ( $product->enable_dimensions_display() && ( $product->has_dimensions() || $product->has_weight() ) ) ) ) {
			$tabs['additional_information'] = array(
				'title'    => __( 'Additional Information', 'nonprofit' ),
				'priority' => 20,
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				'title'    => sprintf( __( 'Reviews (%d)', 'nonprofit' ), $product->get_review_count() ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_related_products' ) ) {
	...
	function woocommerce_related_products( $args = array(), $columns = false, $orderby = false ) {
		if ( ! is_array( $args ) ) {
			_deprecated_argument( __FUNCTION__, '2.1', __( 'Use $args argument as an array instead. Deprecated argument will be removed in WC 2.2.', 'nonprofit' ) );
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => _x( 'Home', 'breadcrumb', 'nonprofit' )
		) ) );
...
if ( ! function_exists( 'woocommerce_checkout_payment' ) ) {
	...
	function woocommerce_checkout_payment() {
		...
		wc_get_template( 'checkout/payment.php', array(
			...
			'available_gateways' => WC()->payment_gateways()->get_available_payment_gateways(),
			'order_button_text'  => apply_filters( 'woocommerce_order_button_text', __( 'Place order', 'nonprofit' ) )
		) );
	}
}
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( $args['required'] ) {
			...
			$required = ' <abbr class="required" title="' . esc_attr__( 'required', 'nonprofit'  ) . '">*</abbr>';
		} else {
			$required = '';
		}
			...
			case 'country' :
				...
				if ( 1 === sizeof( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . '>'
							. '<option value="">'.__( 'Select a country&hellip;', 'nonprofit' ) .'</option>';
					...
					$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__( 'Update country', 'nonprofit' ) . '" /></noscript>';
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
						<option value="">'.__( 'Select a state&hellip;', 'nonprofit' ) .'</option>';
					...
					foreach ( $args['options'] as $option_key => $option_text ) {
						if ( '' === $option_key ) {
							...
							if ( empty( $args['placeholder'] ) ) {
								$args['placeholder'] = $option_text ? $option_text : __( 'Choose an option', 'nonprofit' );
							}
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		$args = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
			...
			'id'               => '',
			'class'            => '',
			'show_option_none' => __( 'Choose an option', 'nonprofit' )
		) );
